...
| H-004 | Editar `Cliente.nombre` desde la bandeja de WhatsApp (los nombres auto-creados no coinciden con el real). Endpoint `POST /api/whatsapp/conversations/<phone>/editar-nombre/` (luna-key, body `{nombre}`, audita MovimientoCliente) | Django (endpoint) + aremko-cli (proxy Go + UI lápiz) | 🟡 SOLICITADO — contrato completo en `BRIEF_H-004_editar_nombre_cliente.md`. Patrón: igual a `marcar-atendido`. aremko-cli arma proxy + edición inline en el encabezado de `ConversacionWhatsApp` (donde ya viven nombre+teléfono, commit `a942fa2`) cuando Django confirme el endpoint | 2026-06-13 (agente aremko-cli) |
