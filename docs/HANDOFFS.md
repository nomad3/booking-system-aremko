...
| H-005 | "Marcar como leído": que el badge "1" y el filtro "solo pendientes" se basen en `requiere_atencion` (que responder/marcar-atendido ya limpian) y no en timestamp (`last_in > last_out`), para poder cerrar una conversación sin respuesta. + botón ✓ en aremko-cli | Django (lógica lista) + aremko-cli (botón) | 🟢 IMPLEMENTADO (Django `eac641a`) — `conversations`: `_pendiente`=`req>0` y badge `sin_responder`=`req` (basado en `requiere_atencion`, no timestamp); reacciones (`type='reaction'`) ya no marcan pendiente. En prod. Falta aremko-cli: botón "✓ marcar como leído" → `marcar-atendido` existente → CERRADO. Ver `RESPUESTA_H-005` | 2026-06-13 (agente Django) |
