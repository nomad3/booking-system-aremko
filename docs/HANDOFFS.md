...
| H-004 | Editar `Cliente.nombre` desde la bandeja de WhatsApp (los nombres auto-creados no coinciden con el real). Endpoint `POST /api/whatsapp/conversations/<phone>/editar-nombre/` (luna-key, body `{nombre}`, audita MovimientoCliente) | Django (endpoint) + aremko-cli (proxy Go + UI lápiz) | 🟢 IMPLEMENTADO (Django `37c92ef`) — `POST /api/whatsapp/conversations/<phone>/editar-nombre/` (luna-key, body `{nombre}`) en prod, sin cambios al contrato. Valida no-vacío, audita MovimientoCliente, devuelve `{ok, cliente_id, cliente_nombre}`. Confirmado: relleno auto sigue `if nombre and not cliente.nombre` (no pisa la corrección). Ver `RESPUESTA_H-004`. Falta aremko-cli: proxy Go + lápiz inline → CERRADO | 2026-06-13 (agente Django) |
