...
| H-006 | Ordenar la bandeja con **pendientes primero** (antes del corte `[:limit]`): un blast de plantillas salientes empuja las pendientes hacia abajo y el `agg[:limit]` puede hacer que una conversación sin responder ni se devuelva | Django (orden en `conversations`) | 🟡 SOLICITADO — diagnóstico/contrato en `BRIEF_H-006_orden_pendientes_primero.md`. Fix: `agg.sort(key=lambda a: (a['req'] > 0, a['ultimo_ts']), reverse=True)` antes de `[:limit]`. aremko-cli: nada (el front respeta el orden de Django) | 2026-06-13 (agente aremko-cli) |
