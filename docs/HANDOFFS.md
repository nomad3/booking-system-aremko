...
| H-007 | **Agente IA que contesta WhatsApp** (plan por fases): responde SOLO sobre servicios publicados + productos con stock>0; grounding con catálogo vivo; modelo desde env Render, tono/on-off desde formulario aremko-cli; human-in-the-loop primero (F1 borrador → F2 auto-info → F3 auto+botón). Proyecto "Agentes IA Aremko" | Django (núcleo agente + grounding + config) + aremko-cli (UI control + borradores) | 🟡 SOLICITADO — plan completo (arquitectura, fases, reglas transversales, estructura del prompt, contratos de datos, reparto) en `BRIEF_H-007_agente_whatsapp_ia.md`. Recomendación: arrancar por **Fase 1** (borrador asistido) sobre la infra existente; cerrar H-007 con F1 validada, F2/F3 como iteraciones | 2026-06-13 (agente aremko-cli) |
