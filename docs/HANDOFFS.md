...
| H-008 | Botón **"Mensaje de ausencia"**: toggle manual que, al activarse, auto-responde a cada entrante una frase fija ("solo atendemos por www.aremko.cl…"); al desactivarse vuelve el flujo normal (Deborah/borrador agente) | Django (auto-respuesta en inbound + config `ausencia_activa`/`ausencia_mensaje`) + aremko-cli (toggle + editor) | 🟢 IMPLEMENTADO (Django) 2026-06-13 — `whatsapp_agent` suma `ausencia_activa`/`ausencia_mensaje`/`ausencia_anti_spam_horas` a la config (en `agente/config`) + tabla `AusenciaEnviada` (anti-spam por teléfono). El inbound devuelve directiva `responder_ausencia: {mensaje}|null` (guard anti-spam por conversación cada N h; default 6); **quién envía = Go de aremko-cli** (`SendSessionMessage`). Precedencia: con ausencia activa, `sugerencia_agente`=null (no gasta LLM). `outbound` acepta `no_marcar_atendido:true` para que el saliente de ausencia NO saque la conversación de pendientes (Deborah las ve al volver). Migración aditiva (`0002`, 3 campos + 1 tabla). Falta: deploy + `migrate whatsapp_agent` en Shell + lado aremko-cli (toggle/editor + Go envía la directiva y persiste con `no_marcar_atendido`). Ver `RESPUESTA_H-008` | 2026-06-13 (agente Django) |
