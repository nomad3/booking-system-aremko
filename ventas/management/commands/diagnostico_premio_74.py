...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(f"  Fecha ganado (creación): {premio.fecha_ganado}")
            self.stdout.write(f"  Fecha aprobación: {premio.fecha_aprobacion or 'Pendiente'}")
            self.stdout.write(f"  Fecha enviado: {premio.fecha_enviado or 'No enviado'}")
            self.stdout.write(f"  Fecha expiración: {premio.fecha_expiracion}")
            self.stdout.write(f"  Estado: {premio.estado}")
            self.stdout.write(f"  Código único: {premio.codigo_unico}")
