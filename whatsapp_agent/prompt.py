...
def build_system_prompt(persona_tono, catalogo_texto, link_reserva, conocimiento='', fecha_hoy='',
    ...
    if fecha_hoy:
        bloque_disponibilidad = (
            ...
            'a YYYY-MM-DD con la fecha de hoy). IMPORTANTE: si el cliente pregunta de forma ambigua (ej. "para el sábado" '
            'sin día específico), EXPLÍCITAMENTE en tu respuesta aclara cuál es la fecha resuelta (ej. "Para el sábado 20 tengo..."). '
            'Si es muy ambiguo, pregunta primero. SIEMPRE lista TODAS las opciones disponibles de esa consulta — no ofrezcas solo una. '
            ...
            '(ej. "cabaña del 24 al 27", "2 noches en cabaña"), DESAMBIGUA PRIMERO el nº de noches si '
            'no está 100% claro (ej. cliente dice "del 24 al 27" → "¿3 noches [entrada 24, salida 27] '
            'o algo distinto?"). Una vez claro, usa `consultar_disponibilidad_alojamiento_multinoche` '
            'con `fecha_llegada` (check-in), `noches` (entero ≥1), y `personas` (1-2). '
            'Devuelve `cabanas` = cabañas libres en TODAS las noches, cada una con `total_por_noche` '
            '(tarifa plana) y `total_estadia`. Presenta compacto (nombre + total_estadia) y pregunta '
            'cuál prefiere. NO muestres `precio_por_persona` (nunca lo incluyas). Menciona SIEMPRE: '
            '"check-in 16:00 [fecha_llegada], check-out 11:00 [fecha_salida]", nº de noches y total. '
            'Usa los montos TAL CUAL. Si ninguna cabaña está libre, ofrece alternativas (futuro).'
        )
