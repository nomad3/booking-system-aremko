...
def build_system_prompt(persona_tono, catalogo_texto, link_reserva, conocimiento='', fecha_hoy='',
    ...
    if fecha_hoy:
        bloque_disponibilidad = (
            ...
            'el masaje (`masaje.hora`). **CUANDO EL CLIENTE ELIJA UNA OPCIÓN, agregá al carrito la tina Y '
            'el masaje de ESA opción llamando `agregar_servicio_carrito` con el `servicio_id` y la `hora` '
            'TAL CUAL los devolvió la herramienta (`tina.servicio_id`/`tina.hora` y '
            '`masaje.servicio_id`/`masaje.hora`). NO resuelvas el masaje por nombre: el `masaje.servicio_id` '
            'del pack es el que tiene ese horario exacto; si pasás otro, la hora se rechaza como inválida.** '
            'PRECIO por opción: si `hay_descuento`, muestra AMBOS — el precio '
