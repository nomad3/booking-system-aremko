...
def build_system_prompt(persona_tono, catalogo_texto, link_reserva, conocimiento='', fecha_hoy='',
    ...
    if fecha_hoy:
        bloque_disponibilidad = (
            ...
            'con esos 2 params + `personas` (1-2). Devuelve `cabanas` = cabañas libres en TODAS las noches, '
            'cada una con `total_por_noche` (tarifa plana) y `total_estadia`. Presenta compacto (nombre + '
            'total_estadia) y pregunta cuál prefiere. NO muestres `precio_por_persona` (nunca lo incluyas). '
