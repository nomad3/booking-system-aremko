#!/usr/bin/env python3
"""
Script para CONTINUAR importación desde el registro 15,501
SALTA los primeros 15,500 registros ya importados
Incluye RECONEXIÓN AUTOMÁTICA para manejar timeouts SSL
"""
import csv
import os
import sys
...
SKIP_ROWS = 15500 # Saltar primeros 15,500 registros (ya importados)
...
def importar_servicios_continuacion():
    print("=" * 70)
    print(f"📊 CONTINUACIÓN DE IMPORTACIÓN - DESDE FILA {SKIP_ROWS + 1}")
    print("=" * 70)
    print()
    ...
    print(f"   ✅ Saltando primeros {SKIP_ROWS:,} registros del CSV")
    print()

    # 2. Procesar CSV desde el registro 15,501
    print(f"📋 PASO 2: Procesando desde fila {SKIP_ROWS + 1}...")
    ...
    with open(CSV_FILE, 'r', encoding='utf-8') as f:
        ...
        for idx, row in enumerate(reader, 1):
            ...
            # SALTAR los primeros 15,500 registros
            if idx <= SKIP_ROWS:
                stats['registros_saltados'] += 1
                continue

            # Reportar progreso cada 1000 registros PROCESADOS (no saltados)
            if (idx - SKIP_ROWS) % 1000 == 0:
                print(f"   📊 Procesando fila CSV {idx:,} (nuevos: {stats['servicios_importados']:,})...")
            ...
            try:
                ...
                while retry_count < MAX_RETRIES:
                    try:
                        ...
                        if cliente_id:
                            # Cliente encontrado - insertar servicio directo (sin verificar duplicados)
                            cur.execute('''
                        ...
                        if batch_count >= BATCH_SIZE:
                            conn.commit()
                            lote_actual += 1
                            print(f"   ✅ Lote {lote_actual}: {batch_count} servicios guardados (Total nuevos: {stats['servicios_importados']:,}, Fila CSV: {idx:,})")
                            batch_count = 0
    ...
    print(f"Total filas en CSV: {stats['total_csv']:,}")
    print(f"⏭️  Registros saltados: {stats['registros_saltados']:,}")
    ...
    print(f"Incremento: +{count_final - count_actual:,}")
    print()
...
if __name__ == '__main__':
    try:
        importar_servicios_continuacion()
    except Exception as e:
        print(f"\n❌ ERROR FATAL: {e}")
        import traceback
        traceback.print_exc()
